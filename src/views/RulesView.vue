  ...
  <main class="flex flex-col p-10 overflow-y-scroll h-full" v-if="loaded">
    ...
    <div>
      <table class="w-full bg-white rounded-xl shadow-lg">
        ...
        >
          <td
            class="font-light pl-6 text-gray-800"
            v-on:click="evalRule(rule.id)"
          >
            {{ index + 1 }}
          </td>
          <td
            class="font-semibold text-gray-800"
            v-on:click="evalRule(rule.id, rule.ruleNaturalLanguage, rule.name)"
          >
            {{ rule.name }}
          </td>
    ...
    <div v-if="registers.length > 0" class="mb-20">
      <h1 class="font-bold text-3xl text-indigo-900 mb-4 mt-4 mb-6">Result:</h1>
      <div
        class="
          flex
          bg-indigo-100/50
        ...
        "
      >
        <h1 class="text-gray-800 text-lg font-semibold mr-4">
          {{ currentName }}:
        </h1>
      ...
      <div class="p-8 bg-indigo-100/50 rounded-xl shadow-lg">
        <table class="w-full bg-white rounded-xl shadow-lg">
          <tr class="border-b h-16 text-left text-indigo-900">
            <th
              class="pl-6"
              v-for="(key, index) in Object.keys(registers[0])"
              :key="index"
            >
              {{ key }}
            </th>
